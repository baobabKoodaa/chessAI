...
public class BlackMagicV2 extends Evaluator {
    ...
    public static final double IMMOBILIZED_KING_BONUS = 40;
    public static final double MOBILITY_BONUS = 10;
    public static final double PROTECTION_BONUS = 5;
    ...
    public void analyzePotentialCapturesEtc() {
        ...
        for (int x=0; x<6; x++) {
            for (int y=0; y<6; y++) {
                ...
                /* Bonus for protected units */
                double prot = PROTECTION_BONUS * sqControlCount[defendColor][x][y];
    ...
    public void scoreMobilityAndCaptures() {
        ...
        if (!kingIsInCheck(sideToMove)) { 
            ...
            *       valuable potential capture, which will never be realized. */
            ...
            /* sideToMove is in check, so it has earlier been penalized with COST_OF_TEMPO
            *   Award the 2nd best naive capture value rather than the best
            *   TODO: Deal with case where best capture also ends the check */
            double v = capBest[sideToMove][1];
            score[sideToMove] += v;
            if (STATE == DEBUG) {
            ...
            score[sideWaiting] += capBest[sideWaiting][0];
            if (STATE == DEBUG) {
                scoreAnalyzer[sideWaiting].add("Naive value for 2nd best capture", "--", capBest[sideWaiting][0]);
            }
        }
    ...
    public void postProcessKing(int color, int unitId) {
        ...
        for (int i=1; i<neighboringSquares[ax][ay][0];) {
            ...
            if (isFriendly[color][sq]) continue;
            availableMoves++; /* Empty or undefended enemy piece */
        }
        ...
        if (enemyControlNearKing > 0) {
            score[enemyColor] += enemyControlNearKing;
            if (availableMoves == 0) {
                score[enemyColor] += IMMOBILIZED_KING_BONUS;
                if (STATE == DEBUG) scoreAnalyzer[enemyColor].add("Near enemy king", "Immobilized king bonus", IMMOBILIZED_KING_BONUS);
            }
        }
    }
    ...
    private void preProcessRook(int color, int ax, int ay) {
        ...
        if (pinned[ax][ay][0] != 0) {
            ...
            if (x == ax || y == ay) {
                mobilityCount[color]++;
                addControl(color, PVrook, x, y);
            }
            return; /* Note: additional allowed moves may exist... */
        }
        addHorizontalVerticalControlsAndMobility(color, PVrook, ax, ay);
    }
    ...
    private void addHorizontalVerticalControlsAndMobility(int color, double unitValue, int ax, int ay) {
        ...
        for (; x>=0; x--) {
            ...
            if (sq != Position.Empty) {
                if (isEnemy[color][sq]) mobilityCount[color]++;
                break;
            }
        }
        x = ax+1;
    ...
    private void addControlsDiagonal(int color, double unitValue, int ax, int ay) {
        ...
        while (x <= 5 && y <= 5) {
            addControl(color, unitValue, x, y);
            if (b[x][y] != Position.Empty) {
                if (isEnemy[color][b[x][y]]) mobilityCount[color]++;
                break;
            }
            x++;
            y++;
    ...
    private void preProcessKnight(int color, int ax, int ay) {
        ...
        for (Square move : knightMoveLists[ax][ay]) {
            int x = move.x;
            int y = move.y;
            addControl(color, PVknight, x, y);
            if (!isFriendly[color][b[x][y]]) mobilityCount[color]++;
        }
    }
    ...
    private void preProcessPawn(int color, int ax, int ay) {
        ...
        if (x >= 0) {
            sqControlCount[color][x][y]++;
            sqControlLowVal[color][x][y] = PVpawn;
            if (isEnemy[color][b[x][y]]) mobilityCount[color]++;
        }
        x = ax + 1;
        if (x <= 5) {
